...
table1 <- t(c(white.p, black.p, srs.se, 
              (white.p-black.p) + z.95 * srs.se * c(-1, 1)))
...
for (i in 1:nrow(openings.valid.str.sample)) {
  ...
  estimates[i] <- white.p.str.est - black.p.str.est
  # Using pooled variance
  diff.se <- sqrt(((n.d.s1-1)*white.str.se^2 + 
                     (n.d.s2-1)*black.str.se^2)/(n.d.s1+n.d.s2-2))
...
srs.se <- sqrt((1-n.w/N)*var(white.win$turns)/n.w + 
                 (1-n.b/N)*var(black.win$turns)/n.b)
(white.win.turns.avg - black.win.turns.avg) + z.95 * srs.se * c(-1, 1)
```
