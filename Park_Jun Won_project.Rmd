...
# Stratified sampling
df$elo_range <- cut(df$average_elo, c(1200, 1400, 1600, 1800, 2000, 2200, 2400, 2600))
levels(df$elo_range) <- c("1200-1400", "1400-1600", "1600-1800", "1800-2000", 
                          "2000-2200", "2200-2400", "2400+")

# Sample from each strata using proportional allocation
stratified_sample <- df %>% 
  group_by(elo_range) %>%
  sample_frac(round(n/nrow(df), 4), replace = FALSE)
